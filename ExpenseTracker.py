...
def deleteallusers():
    if profilename == "":  # Check if no user is logged in
        messagebox.showwarning('Access Denied', 'You must be logged in to delete all users.')
    else:
        response = messagebox.askyesno('Confirm Delete', 'Are you sure you want to delete all users?')
        if response:
            conn = sqlite3.connect("loginpage.db")
            cur = conn.cursor()
...
def appwindow():
    ...
    def endpage():
        ...
        Label(gui,font=("lucida fax",40),bg="#bfbfbf",text="My Money Mirror").place(x=190,y=10)
        ...
        Label(gui,font=("ink free",35),bg="#bfbfbf",text="Kaviya").place(x=500,y=450)
    ...
    b3=Button(gui,text="Delete with id",font=("georgia",17),activebackground="#fffa66",activeforeground="red",width=12,command=deletewithid).place(x=572,y=250)
    ...
    l6=Label(gui,width=60,font=("century",35),bg="#ff9999",fg="#b32d00",text="My Money Mirror").place(x=-450,y=0)
...
b2=Button(root,text="Sign up",font=("algerian",19),activebackground="#fffa66",activeforeground="red",width=12,command=register).place(x=630,y=400)
...
Label(root,width=60,font=("jokerman",35),bg="#ff9999",fg="#cc2900",text="My Money Mirror").place(x=-400,y=0)
Label(root,font=("Ink Free",27),bg="#33cccc",fg="#0000ff",text="Kaviya").place(x=750,y=590)
